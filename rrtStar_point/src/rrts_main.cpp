...
bool generatePath(rrtstar_msgs_point::rrtStarSRV::Request &req, rrtstar_msgs_point::rrtStarSRV::Response &res){
//! request Values:
	...
	z[0]=req.Init.x; 	z[1]=req.Init.y; 	z[2]=req.Init.z;
	...
	for (int i=0; i< req.Obstacles.size(); ++i){
	      const rrtstar_msgs_point::Region &data = req.Obstacles[i];
	      cout<<"req.Obstacles("<<i<<")"<<data.center_x<<" "<<data.center_y<<" "<<data.center_z<<" "
	    		  <<data.size_x<<" "<<data.size_y<<" "<<data.size_z<<" "<<endl;
    }
//! rrtStar Method:
	...
	for (int i=0; i< req.Obstacles.size(); ++i){
		const rrtstar_msgs_point::Region &obs = req.Obstacles[i];
	    region *obstacle;
	    obstacle = new region;
...
int main (int argc, char** argv) {
	...
	ros::init(argc, argv, "rrtstar_point");
	ros::NodeHandle nh;
	ros::ServiceServer service = nh.advertiseService("rrtStar_point_Service",generatePath);
